...
extension String: LKExCompatible {}

// MARK: - 一：字符串基本的扩展
public extension LKEx where Base: ExpressibleByStringLiteral {
    ...
    func toClass() -> AnyClass? {
        ...
        guard let Class: AnyClass = NSClassFromString(namespace.ex.removeSomeStringUseSomeString(removeString: " ", replacingString: "_") + "." + (base as! String)) else {
            return nil
        }
        return Class
    }
...
public extension LKEx where Base: ExpressibleByStringLiteral {
    ...
    var toNumber: NSNumber? {
        return self.toDouble()?.ex.number
    }
}

// MARK: - 五、字符串UI的处理
extension LKEx where Base: ExpressibleByStringLiteral {
...
extension LKEx where Base: ExpressibleByStringLiteral {
    ...
    public var isValidMobile: Bool {
        let rgex = "^(?:\\+?86)?1(?:3\\d{3}|5[^4\\D]\\d{2}|8\\d{3}|7(?:[0-35-9]\\d{2}|4(?:0\\d|1[0-2]|9\\d))|9[0-35-9]\\d{2}|6[2567]\\d{2}|4[579]\\d{2})\\d{6}$"
        return predicateValue(rgex: rgex)
    }
    ...
    public var isValidIDCardNumStrict: Bool {
        ...
        case 15:
            ...
            year = Int(str.ex.sub(start: 6, length: 2))!
        ...
        case 18:
            // 18位身份证
            year = Int(str.ex.sub(start: 6, length: 4))!
            ...
            if numberOfMatch > 0 {
                ...
                for i in 0 ..< 17 {
                    if let d = Int(str.ex.slice(i ..< (i + 1))) {
                        s += d * jiaoYan[i % 10]
                    } else {
                        return false
                    }
                ...
                let Y = s % 11
                let JYM = "10X98765432"
                let M = JYM.ex.sub(start: Y, length: 1)
                if M == str.ex.sub(start: 17, length: 1) {
                    return true
                } else {
                    return false
                }
    ...
    public func stringWithHighLightSubstring(keyword: String, normalColor: UIColor, keywordCololor: UIColor, isSplit: Bool = false, options: NSRegularExpression.Options = []) -> NSMutableAttributedString {
        ...
        if isSplit {
            for i in 0..<keyword.count {
                let singleString = keyword.ex.sub(start: i, length: 1)
...
extension LKEx where Base: ExpressibleByStringLiteral {
    ...
    public func nsRange(of subString: String) -> NSRange? {
        guard (base as! String).ex.contains(find: subString) else {
            return nil
        }
    ...
    public func insertString(content: String, locat: Int) -> String {
        ...
        let str1 = (base as! String).ex.sub(to: locat)
        let str2 =  (base as! String).ex.sub(from: locat + 1)
...
public extension LKEx where Base: ExpressibleByStringLiteral {
    ...
    func binaryToOctal() -> String {
        ...
        // 八进制
        return decimal.ex.decimalToOctal()
    }
    ...
    func binaryToHexadecimal() -> String {
        ...
        let decimal = binary.binaryTodecimal()
        // 十六进制
        return decimal.ex.decimalToHexadecimal()
    }
    ...
    func octalTobinary() -> String {
        ...
        // 二进制
        return decimal.ex.decimalToBinary()
    }
    ...
    func octalToHexadecimal() -> String {
        ...
        // 十进制
        let decimal = octal.ex.octalTodecimal()
        // 十六进制
        return decimal.ex.decimalToHexadecimal()
    }
    ...
    func decimalToBinary() -> String {
        guard var decimal = (self.base as! String).ex.toInt() else {
            return ""
        }
        var str = ""
        while decimal > 0 {
    ...
    func decimalToOctal() -> String {
        guard let decimal = (self.base as! String).ex.toInt() else {
            return ""
        }
        /*
...
public extension LKEx where Base: ExpressibleByStringLiteral {
    ...
    func md5Encrypt() -> String? {
        ...
        if let jet = (self.base as! String).data(using: .utf8) {
            return Insecure.MD5.hash(data: jet)
