...
extension UIDevice: LKExCompatible {}
// 这里只指屏幕类型
...
// MARK: - 一、基本的扩展
public extension LKEx where Base: UIDevice {
    ...
    static func screenType() -> UIDeviceScreenType {
        let modelName = Base.ex.modelName
    ...
    static func isIphone5Screen() -> Bool {
        if Base.ex.screenType() == .IPHONE_5 {
            return true
        }
        return false
    }
    ...
    static func isIphone6Screen() -> Bool {
        if Base.ex.screenType() == .IPHONE_6 {
            return true
        }
        return false
    }
    ...
    static func isIphoneXScreen() -> Bool {
        if Base.ex.screenType() == .IPHONE_X || Base.ex.screenType() == .IPHONE_XS || Base.ex.screenType() == .IPHONE_XR || Base.ex.screenType() == .IPHONE_XS_Max ||  Base.ex.screenType() == .IPHONE_11 ||  Base.ex.screenType() == .IPHONE_11_PRO ||  Base.ex.screenType() == .IPHONE_11_PRO_MAX {
            return true
        }
        return false
    }
    ...
    static func isIphoneXSScreen() -> Bool {
        if Base.ex.screenType() == .IPHONE_XS || Base.ex.screenType() == .IPHONE_XR || Base.ex.screenType() == .IPHONE_XS_Max {
            return true
        }
        return false
    }
...
public extension LKEx where Base: UIDevice {
    ...
    static var isJailbroken: Bool {
        ...
        let path = String(format: "/private/%@", String.ex.stringWithUUID() ?? "")
        do {
