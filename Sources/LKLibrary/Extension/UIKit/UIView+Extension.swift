...
extension UIView: LKExCompatible {}
// MARK: - 一、机型的判断
/*
 iphone硬件型号
...
/**
 ...
 m44:初始为1
 */
extension LKEx where Base: UIView {
...
public extension LKEx where Base: UIView {
    ...
    func drawCircle(fillColor: UIColor, strokeColor: UIColor, strokeWidth: CGFloat) {
        let ciecleRadius = self.base.ex.width > self.base.ex.height ? self.base.ex.height : self.base.ex.width
    ...
    func drawDashLine(strokeColor: UIColor,
        ...
        // 线粗
        let lineWidth = direction == .horizontal ? self.base.ex.height : self.base.ex.width
        ...
        if direction == .horizontal {
            ...
            // 终点
            // 横向 y = lineWidth / 2
            path.addLine(to: CGPoint(x: self.base.ex.width, y: lineWidth / 2))
        } else {
            path.move(to: CGPoint(x: lineWidth / 2, y: 0))
            // 终点
            // 纵向 Y = view 的height
            path.addLine(to: CGPoint(x: lineWidth / 2, y: self.base.ex.height))
        }
...
public extension LKEx where Base: UIView {
    ...
    func addWater(markText: String, textColor: UIColor = UIColor.black, font: UIFont = UIFont.systemFont(ofSize: 12)) {
        let waterMark: NSString = markText.ex.toNSString
        let textSize: CGSize = waterMark.size(withAttributes: [NSAttributedString.Key.font : font])
        // 多少行
        let line: NSInteger = NSInteger(self.base.ex.height * 3.5 / 80)
        // 多少列：自己的宽度/(每个水印的宽度+间隔)
        let row: NSInteger = NSInteger(self.base.ex.width / markText.ex.rectWidth(font: font, size: CGSize(width: self.base.ex.width, height: CGFloat(MAXFLOAT))))
        for i in 0..<line {
            for j in 0..<row {
    ...
    func isContainsWKWebView() -> Bool {
        ...
        for subView in base.subviews {
            if (subView.ex.isContainsWKWebView()) {
                return true
            }
        }
        return false
...
public extension LKEx where Base: UIView {
    ...
    func getViewLayer(borderWidth: CGFloat = 0.5, borderColor: UIColor = .randomColor, backgroundColor: UIColor = .randomColor) {
        ...
        for subview in subviews {
            ...
            subview.ex.getViewLayer(borderWidth: borderWidth, borderColor: borderColor, backgroundColor: backgroundColor)
        }
        #endif
    }
    ...
    func findSubview(type: UIResponder.Type, resursion: Bool)-> UIView? {
        ...
        for e in self.base.subviews.enumerated() {
            let tmpView = e.element.ex.findSubview(type: type, resursion: resursion)
            if tmpView != nil {
                return tmpView
            }
        }
...
public extension LKEx where Base : UIView {
    ...
    func gradientColor(_ direction: JKViewGradientDirection = .horizontal, _ gradientColors: [Any], _ gradientLocations: [NSNumber]? = nil) {
        // 获取渐变对象
        let gradientLayer = CAGradientLayer().ex.gradientLayer(direction, gradientColors, gradientLocations)
        // 设置其CAGradientLayer对象的frame，并插入view的layer
        gradientLayer.frame = CGRect(x: 0, y: 0, width: self.base.ex.width, height: self.base.ex.height)
    ...
    func gradientColorAnimation(direction: JKViewGradientDirection = .horizontal, startGradientColors: [Any], endGradientColors: [Any], duration: CFTimeInterval = 1.0, gradientLocations: [NSNumber]? = nil) {
        // 获取渐变对象
        let gradientLayer = CAGradientLayer().ex.gradientLayer(direction, startGradientColors, gradientLocations)
        // 设置其CAGradientLayer对象的frame，并插入view的layer
        gradientLayer.frame = CGRect(x: 0, y: 0, width: self.base.ex.width, height: self.base.ex.height)
        self.base.layer.insertSublayer(gradientLayer, at: 0)
