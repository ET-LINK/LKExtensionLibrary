...
public struct JKGlobalTools {
    ...
    public static func compareVersion(version: String) -> Bool {
        ...
        // 2、当前版本的版本号获取 三位Int值
        let currentVersion = Bundle.ex.appVersion
...
private extension JKGlobalTools {
    ...
    static func appVersion(version: String) -> (isSuccess: Bool, versions: [Int]) {
        let versionArray = version.ex.separatedByString(with: ".")
        ...
        guard let versionValue1 = versionString1.ex.toInt(), let versionValue2 = versionString2.ex.toInt(), let versionValue3 = versionString3.ex.toInt() else {
            return (false, [])
        }
