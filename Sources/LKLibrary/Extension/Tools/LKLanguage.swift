...
// MARK: - 多语言的一些设定和操作扩展
public extension LKLanguage {

    /// 默认语言
    static var `default`: LKLanguage = .zh_CN

    /// 当前语言
    static var current: LKLanguage = .zh_CN {
        didSet {
            _refresh(current)
        }
    }
    ...
    private static func _refresh(_ language: LKLanguage = .default) {
...
public extension LKLanguage {
    static var isChinese: Bool {
        return LKLanguage.current == .zh_CN
    }
}
...
extension LKLanguage {
    fileprivate static func detectLanguage() -> LKLanguage {
        if isSystemChineseSimple() {
            return LKLanguage.zh_CN
        }
        if let currentLanguage = Locale.preferredLanguages.first,
            let country = currentLanguage.split(separator: Character("-")).first,
            let lang = LKLanguage(rawValue: String(country)) {
            return lang
        }
        return .en
    }
